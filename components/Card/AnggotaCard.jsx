...
export default function AnggotaCard({ data, session }) {
  const nama = useRef("");
  const kelas = useRef("");
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle className="text-md font-semibold">{data.nama}</CardTitle>
        <CardDescription>{data.kelas}</CardDescription>
      </CardHeader>
      <CardContent>
        <div>
          <h3>
            Gender {data.gender === "Laki_Laki" ? "Laki Laki" : data.gender}
          </h3>
      ...
      {session?.role === "Admin" && (
        <CardFooter>
          <DropdownMenu>
            <DropdownMenuTrigger>
              <MdMoreHoriz className="text-right" size={20} />
            </DropdownMenuTrigger>
